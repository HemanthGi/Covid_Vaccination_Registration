    ...
    if session.get('name'):
        if(session.get('name')=='admin'):
            if request.method == "POST":
                ...
                if issues.find_one({ 'location':request.form['location'], 'date':request.form['date'], 'details':request.form['details'], 'name':session.get('name'), 'email':session.get('email')}):
                    ...
                    issues.insert_one({ 'location':request.form['location'], 'date':request.form['date'], 'details':request.form['details'], 'name':session.get('name'), 'email':session.get('email')})
                    flash("Slot Added","success")
                    return redirect("/issues")
            return render_template('addissue.html')
        else:
            flash("You are able to add location","danger")
            return redirect("/issues")
...
def editissue(idx=None):
    if session.get('name'):
        if request.method == 'POST':
                issues.find_one_and_update({'_id':ObjectId(idx)},{"$set":{'location':request.form['location'], 'date':request.form['date'], 'details':request.form['details'], 'name':session.get('name'), 'email':session.get('email')}})
